...
> 트랜잭션(Transcation)은 데이터베이스의 상태를 변환시키는 하나의 논리적 기능을 수행하기 위한 작업의 단위 또는 한꺼번에 모두 수행되어야 할 일련의 연산들을 의미한다.

- 트랜잭션은 데이터베이스 시스템에서 병행 제어 및 회복 작업 시 처리디는 작업의 논리적 단위이다.
...
- 트랜잭션의 연산은 데이터베이스에 모두 반영되던가 전혀 반영되지 않아야 한다.
...
- 실패 (Falied) : 트랜잭션 실행에 오류가 발생하여 중단된 상태
